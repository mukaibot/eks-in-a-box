...
module Prereqs
  class ClientBinaryInstaller
    ...
    KUBECTL_VERSION = '1.11.5/2018-12-06'
    ...
    def client_binaries
      [
        {
          name: 'kubectl',
          url:  "https://amazon-eks.s3-us-west-2.amazonaws.com/#{KUBECTL_VERSION}/bin/#{platform}/amd64/kubectl"
        },
        {
          name: 'aws-iam-authenticator',
          url:  "https://amazon-eks.s3-us-west-2.amazonaws.com/#{KUBECTL_VERSION}/bin/#{platform}/amd64/aws-iam-authenticator"
        },
        {
          name: 'eksctl',
    ...
    def install(bin)
      ...
      logger.info "Installing #{name} to #{dest}"
    ...
    def go_bin(bin, dest)
      `curl --silent --location "#{bin.fetch(:url)}" -o #{dest}`
      FileUtils.chmod(755, dest)
    end
  end
end
